...
def main(args):
    ...
    def update(frame=None):
        ...
        for ipar, (name, minimum, maximum, logscale, lbound, rbound, marginal, ax) in enumerate(zip(parnames, parmin, parmax, parlog, lbounds, rbounds, marginals, axes)):
            ...
            # Add title
            # TKA: print only par name
            gotm_par = ['surface', 'light_extinction']
            number_split = name.count('/')
            instance = name.split('/', number_split)[1]
            if instance in gotm_par:
                print_name = name.split('/', number_split-1)[-1]
            elif instance == 'instances':
                module, par_name = name.split('/', number_split)[2], name.split('/', number_split)[-1]
                print_name = par_name #module+"/"+par_name
            else:
                print_name = name.split('/', number_split)[-1]
            ax.set_title(print_name) # fontsize=8
            ...
            #TKA: set axis and title font size
